...
try {
    ...
    // Edit the real native export, leaving its original integrity manifest intact.
    const expected = structuredClone(source);
    const edited = expected.envelope.entities[0];
    edited.title = 'Rêve « modifié » — portable';
    edited.description += '\n\nModification extérieure : conserver relations, historique et pièces jointes.\n';
    const manifest = JSON.parse(new TextDecoder().decode(second.get(PORTABLE_MANIFEST)));
    const object = manifest.files.find(entry => entry.kind === 'object' && entry.entityId === edited.id);
    assert.ok(object);
    const markdown = new TextDecoder().decode(second.get(object.path));
    const changed = markdown.replace(/^title: .+$/m, `title: ${JSON.stringify(edited.title)}`)
        .replace(/(\n---\n)[\s\S]*$/, (_, separator) => separator + edited.description);
    await writeFile(join(vault, secondExport, object.path), changed, 'utf8');
    await page.evaluate(() => app.commands.executeCommandById('dreamglows:import-portable'));
    dialogPage = page;
    for (const candidate of browser.contexts()[0].pages()) if (await candidate.getByRole('heading', { name: 'Restaurer un paquet DreamGlows' }).count()) dialogPage = candidate;
    await dialogPage.getByRole('textbox', { name: 'Dossier du paquet' }).fill(secondExport);
    await dialogPage.getByRole('button', { name: 'Prévisualiser', exact: true }).click();
    await dialogPage.getByRole('alert').waitFor();
    assert.match(await dialogPage.getByRole('alert').innerText(), /Intégrité incorrecte/);
    assert.equal(await dialogPage.getByRole('button', { name: 'Sauvegarder puis restaurer', exact: true }).count(), 0);
    assert.deepEqual(JSON.parse(await readFile(join(pluginPath, 'data.json'), 'utf8')), source);
    report.strictRefusal = 'passed';
    await dialogPage.getByLabel('Accepter les titres et descriptions modifiés en Markdown').click();
    await dialogPage.getByRole('button', { name: 'Prévisualiser', exact: true }).click();
    await dialogPage.getByRole('button', { name: 'Sauvegarder puis restaurer', exact: true }).waitFor();
    assert.match(await dialogPage.locator('.modal-content').filter({ has: dialogPage.getByRole('heading', { name: 'Restaurer un paquet DreamGlows' }) }).innerText(), /1 objets Markdown modifiés/);
    await dialogPage.screenshot({ path: join(evidence, 'edited-import-preview.png') });
    const backupsBefore = await readdir(join(vault, 'DreamGlows-portable'));
    await dialogPage.getByRole('button', { name: 'Sauvegarder puis restaurer', exact: true }).click();
    await dialogPage.getByRole('heading', { name: 'Restaurer un paquet DreamGlows' }).waitFor({ state: 'hidden' });
    assert.deepEqual(JSON.parse(await readFile(join(pluginPath, 'data.json'), 'utf8')), expected);
    const backup = (await readdir(join(vault, 'DreamGlows-portable'))).find(name => name.startsWith('backup-') && !backupsBefore.includes(name));
    assert.ok(backup, 'A new complete destination backup precedes restoration');
    assert.deepEqual(await packageFiles(`DreamGlows-portable/${backup}`), incoming);
    report.backupBeforeEditRestore = 'passed';
    await stop(); await launch();
    const editedExport = await packageFiles(await exportInHost());
    assert.deepEqual(editedExport, await exportPortablePackage(expected, incomingAssets));
    report.editRoundTrip = 'passed';
