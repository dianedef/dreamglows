...
test('GoalsView retains canonical modal entry points without legacy mutations',()=>{assert.match(source,/new EntityModal\(props\.app,uiContext/);assert.match(source,/new EntityModal\(props\.app,uiContext,undefined,'action'/);assert.doesNotMatch(source,/\.(addGoal|updateGoal|deleteGoal|addTask|updateTask|deleteTask)\(/)});
