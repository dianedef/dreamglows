...
🟠 [DreamGlows] task: Définir et versionner le modèle de données ouvert commun | status: in progress | area: product-foundation | id: open-domain-model | depends_on: dreamglows-capability-map | status_note: Contrat v1 complété (why, types, relations, validation et rejeu); Obsidian et Chrome consomment le cœur TypeScript, Windows le package Dart partagé; aller-retour TypeScript → modification Dart → TypeScript prouvé; intégration Android et exposition complète des commandes dans les interfaces restent à faire | acceptance: Schéma documenté pour rêve, pourquoi, objectif, jalon, action, habitude, preuve, bilan, relations, révisions et migrations
